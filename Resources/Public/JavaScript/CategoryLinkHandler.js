...
define(['jquery', 'jquery/autocomplete', 'TYPO3/CMS/Backend/FormEngine', 'TYPO3/CMS/Recordlist/LinkBrowser'], function($, autocomplete, FormEngine, LinkBrowser) {
    ...
    $(function() {
        $('#lshopware-category-wizard').on('focus', function(event) {
            if (!$(this).data('t3-suggest-px-shopware-initialized')) {
                initialize($(this));
                $(this).data('t3-suggest-px-shopware-initialized', true);
            }
        });
        $('#lshopware-category-form').on('submit', function(event) {
            event.preventDefault();

            var value = $(this).find('[name="lshopware-category"]').val();
            if (value === 't3://shopware_category?id=') {
                return;
            }
            if (value.indexOf('t3://shopware_category?id=') === 0) {
                value = value.substr(25);
            }
            LinkBrowser.finalizeFunction('t3://shopware_category?id=' + value);
        });
    });
